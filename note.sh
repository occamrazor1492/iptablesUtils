...
conf=$base/conf
notesFile=$base/notes  # New file to store notes
touch $conf
touch $notesFile  # Ensure the notes file exists
...
addDnat(){
    ...
    local remotehost=
    local valid=
    local note=  # New variable to store the note

    echo -n "本地端口号:" ;read localport
    echo -n "远程端口号:" ;read remoteport
    echo -n "备注/描述 (optional):" ;read note  # Input for the note

    # Save the note in the notesFile
...
LINE
    ...
    echo "成功添加转发规则 $localport>$remotehost:$remoteport  备注: $note 真实情况iptable真实设置，note存储在另外文件不是真实设置文件"
    setupService
}
...
lsDnatNote(){
    echo "---------------"
    echo "备注（以上面显示为准，这个只是设置文件）："
    cat $conf | while read line
    do
        # Fetch the associated note from the notesFile
        note=$(grep "^$line:" $notesFile | cut -d':' -f2)
        echo "$line 备注: $note"
    done
}
